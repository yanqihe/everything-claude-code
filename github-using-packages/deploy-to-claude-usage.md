...
| 2 | `git fetch upstream` + `git rebase upstream/main` + `git push --force-with-lease origin main` | 拉取上游最新代码 → 将本地独有 commits rebasing 到最新上游之上 → 安全强制推送到个人仓库（无 merge commit） |
